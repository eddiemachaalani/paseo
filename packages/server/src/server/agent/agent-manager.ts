...
export class AgentManager {
  ...
  switchAgentProvider(agentId: string, provider: AgentProvider): Promise<ManagedAgent> {
    return this.trackAgentRegistrationOperation(
      this.runLifecycleMutation(agentId, () => this.switchAgentProviderInternal(agentId, provider)),
    );
  }
  ...
  // What a reload needs to know about the provider it will run on: whether the
  // agent moves to another provider entry, the handle to resume under that
  // entry, and which of its last usage still describes the account it lands on.
  private planReload(
    ...
    provider: AgentProvider;
    resumeHandle: ManagedAgent["persistence"];
    preservedLastUsage: ManagedAgent["lastUsage"];
  } {
    ...
    return {
      rehydrateFromDisk: options?.rehydrateFromDisk ?? false,
      provider,
      resumeHandle: handle && providerChanged ? { ...handle, provider } : handle,
      ...
      preservedLastUsage: providerChanged
  ...
  private async reloadAgentSessionInternal(
    ...
    const { rehydrateFromDisk, provider, resumeHandle, preservedLastUsage } = this.planReload(
      existing,
      options,
    );
    ...
    try {
      ...
      session = resumeHandle
        ? await client.resumeSession(resumeHandle, providerLaunchConfig, launchContext)
