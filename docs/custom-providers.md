...
Each profile appears as a separate provider in the Paseo app. You can select which one to use when launching an agent. A running agent can also move to a sibling profile from its tab menu (**Switch provider**): the daemon resumes the same native session under the other profile's credentials, so the conversation continues on that account's quota. See [agent-lifecycle.md](agent-lifecycle.md#runtime-residency).
